...
require("lazy").setup({
	...
	},
	{
		...
		config = function()
			...
			local map = vim.keymap.set -- Nvim DAP
...

	{
		...
		keys = {
			-- Normal mode: Ctrl + /
			{ "<C-_>", "gcc", mode = "n", noremap = true, silent = true, desc = "Toggle comment for current line" },
			-- Visual mode: Ctrl + /
			{
				"<C-_>",
				"gc",
		...
		config = function()
			require("monokai-pro").setup({
				...
				overrideScheme = function(cs, p, config, hp)
					local cs_override = {}
					local calc_bg = hp.blend(p.background, 0.60, "#000000")
